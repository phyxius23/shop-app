...
export function CMSProductForm(props: CMSProductFormProps) {
	...
	return (
		<div
			className={clsx(
				"fixed bg-gray-400 w-full h-full left-0 z-50 transition-all duration-300 overflow-auto",
			...
			>
				...
				<div className="flex flex-col gap-2">
					...
					>
						{props.activeItem?.id ? (
							<>
								<option value={props.activeItem?.console}>
									{props.activeItem?.console}
								</option>
								{console.map(
									(item, index) =>
										props.activeItem?.console !== item && (
											<option
												key={index}
												value={item}
											>
